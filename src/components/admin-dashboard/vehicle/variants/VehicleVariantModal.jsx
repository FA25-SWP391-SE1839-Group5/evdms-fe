...
export default function VehicleVariantModal({ show, onClose, onSaveSuccess, variantToEdit }) {
    ...
    if (show && models.length === 0) {
      ...
      const fetchModels = async () => {
        ...
        try {
          ...
          setError(err + "Failed to load vehicle models. Cannot add/edit variants.");
        } finally {
  ...
  }, [models.length, show]); // Phụ thuộc vào show
  ...
  const handleSpecChange = (specKey, value) => {
    setSpecs((prev) => {
      const newSpecs = { ...prev };
      const specConfig = Object.values(specCategories)
        .flat()
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <form onSubmit={handleSubmit} noValidate onKeyDown={handleFormKeyDown}>
                <div id="wizard-modern-icons" className="bs-stepper wizard-icons wizard-modern mt-2">
                  {/* Stepper Header */}
                  <div className="bs-stepper-header border-bottom">
                    ...
                    <div className={`step ${currentStep >= 2 ? "active" : ""}`} data-target="#specs-step">
                      <button type="button" className="step-trigger" onClick={() => setCurrentStep(2)} disabled={loading}>
                        <span className="bs-stepper-icon">
                    ...
                    <div className={`step ${currentStep >= 3 ? "active" : ""}`} data-target="#features-step">
                      <button type="button" className="step-trigger" onClick={() => setCurrentStep(3)} disabled={loading}>
                        <span className="bs-stepper-icon">
                    ...
                    <div className={`step ${currentStep >= 4 ? "active" : ""}`} data-target="#review-step">
                      <button type="button" className="step-trigger" onClick={() => setCurrentStep(4)} disabled={loading}>
                        <span className="bs-stepper-icon">
                  ...
                  <div className="bs-stepper-content mt-1">
                    ...
                    <div id="basic-details-step" className={`content ${currentStep === 1 ? "active dstepper-block" : "dstepper-none"}`}>
                    ...
                    <div id="specs-step" className={`content ${currentStep === 2 ? "active dstepper-block" : "dstepper-none"}`}>
                      <div className="content-header mb-3">
                          ...
                          <div className="row g-3">
                            {specsInCategory.map((spec) => (
                              <div key={spec.key} className="col-md-6">
                                <label htmlFor={`spec-${spec.key}`} className="form-label">
                                  {spec.label}
                                </label>
                    ...
                    <div id="features-step" className={`content ${currentStep === 3 ? "active dstepper-block" : "dstepper-none"}`}>
                      <div className="content-header mb-3">
                    ...
                    <div id="review-step" className={`content ${currentStep === 4 ? "active dstepper-block" : "dstepper-none"}`}>
                      <div className="content-header mb-3">
                      ...
                      <ul className="list-unstyled">
                        <li>
                          <strong>Model:</strong> {models.find((m) => m.id === basicInfo.modelId)?.name || "N/A"}
                        </li>
                        <li>
                          <strong>Variant Name:</strong> {basicInfo.name || "N/A"}
                        </li>
                        <li>
                          <strong>Base Price:</strong> {basicInfo.basePrice ? new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(basicInfo.basePrice) : "N/A"}
                        </li>
                      </ul>
                      <hr />
                          ...
                          <ul className="list-unstyled row row-cols-md-2">
                            {specCategories[category].map((spec) => (
                              <li key={spec.key} className="col">
                                <small>
                                  {spec.label}: {renderSpecValue(spec.key)}
                                </small>
                              </li>
                            ))}
                          </ul>
                    ...
                    <div className="d-flex justify-content-between p-4 border-top">
                      <button type="button" className="btn btn-primary d-flex align-items-center px-3 py-2" onClick={prevStep} disabled={loading || currentStep === 1}>
                        <ArrowLeft size={16} className="me-1 scaleX-n1-rtl" /> Previous
                      </button>
                      {currentStep < 4 ? (
                        <button type="button" className="btn btn-primary d-flex align-items-center px-3 py-2" onClick={nextStep} disabled={loading || loadingModels}>
