    ...
    <>
      ...
      >
        <div className="offcanvas-header border-bottom">
          <h5 id="auditLogFilterOffcanvasLabel" className="offcanvas-title">
            Filters
          </h5>
        ...
        <div className="offcanvas-body d-flex flex-column">
          ...
          <div className="mb-4">
            <h6>
              <i className="bx bx-time-five me-2"></i>Time Period
            </h6>
            <div className="d-flex flex-wrap gap-2">
              {["all", "today", "last7days", "last30days"].map((period) => (
                <button key={period} type="button" className={`btn btn-sm ${selectedTimePeriod === period ? "btn-primary" : "btn-outline-secondary"}`} onClick={() => setSelectedTimePeriod(period)}>
          ...
          <div className="mb-4">
            <h6>
              <i className="bx bx-user-circle me-2"></i>User Roles
            </h6>
            <div className="d-flex flex-wrap gap-2">
              {ALL_ROLES.map((role) => (
                <div key={role} className="form-check">
                  <input className="form-check-input" type="checkbox" id={`role-${role}`} value={role} checked={selectedRoles.includes(role)} onChange={() => handleRoleChange(role)} />
          ...
          <div className="mb-4">
            <h6>
              <i className="bx bx-list-check me-2"></i>Action Types
            </h6>
            <div className="d-flex flex-wrap gap-2">
              {ALL_ACTIONS.map((action) => (
                <div key={action} className="form-check">
                  <input className="form-check-input" type="checkbox" id={`action-${action}`} value={action} checked={selectedActions.includes(action)} onChange={() => handleActionChange(action)} />
