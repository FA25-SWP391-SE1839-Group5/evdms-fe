...
const AuditLogManagement = () => {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter((log) => {
      ...
      return userName.includes(searchLower) || entityId.includes(searchLower) || action.includes(searchLower) || description.includes(searchLower) || formattedDate.includes(searchLower);
    });
  ...
  return (
    <>
        ...
        <div className="card-header border-bottom">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div className="input-group input-group-merge" style={{ maxWidth: "300px" }}>
              ...
              <input type="search" className="form-control" placeholder="Search logs..." value={searchTerm} onChange={handleSearchChange} aria-label="Search logs" />
            </div>
          </div>
        </div>
                ...
                paginatedLogs.map((log) => {
                  ...
                  return (
                    <tr key={log.id}>
                      <td>
                        <div className="d-flex justify-content-start align-items-center">
                          <div className="avatar avatar-sm me-3">
                            <span className="avatar-initial rounded-circle bg-label-secondary">{getAvatarInitials(user?.name)}</span>
                          </div>
                      ...
                      <td>
                        <button type="button" className="btn btn-icon btn-text-secondary rounded-pill btn-sm" onClick={() => handleViewDetails(log)} title="View Log Details">
      ...
      <AuditLogFilterPanel show={showFilterPanel} onClose={() => setShowFilterPanel(false)} currentFilters={activeFilters} onApplyFilters={handleApplyFilters} />

      {/* Details Modal */}
      <AuditLogDetailsModal show={showDetailsModal} onClose={() => setShowDetailsModal(false)} log={viewingLog} userMap={userMap} />
    </>
  );
};
