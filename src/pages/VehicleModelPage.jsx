...
import { createVehicleModel, deleteVehicleModel, getAllVehicleModels, updateVehicleModel } from "../services/vehicleModelService";
...
const VehicleModelPage = ({ user, onLogout }) => {
  ...
  const handleSubmit = async (data, imageFile) => {
    ...
    try {
      if (editModel) {
        // UPDATE
        const response = await updateVehicleModel(editModel.id, data, imageFile);
        if (response.success && response.data) {
          setModels((prev) => prev.map((m) => (m.id === editModel.id ? response.data : m)));
          alert("Vehicle model updated successfully!");
        }
      } else {
        // CREATE
        const response = await createVehicleModel(data, imageFile);
        if (response.success) {
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={onLogout} className="flex items-center gap-2 px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-xl transition-colors">
              <LogOut size={18} />
              Logout
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        {!showForm && (
          <div className="flex justify-between items-center">
            ...
            <p className="text-sm text-gray-500">{filteredModels.length} model(s) found</p>
          </div>
        )}
        ...
        {showForm ? (
            ...
            initialData={
              editModel
                ? {
                    ...
                    descriptions: editModel.description,
                    imageUrl: editModel.imageUrl,
                    createdAt: editModel.createdAt,
                    updatedAt: editModel.updatedAt,
                  }
                : null
            }
          ...
          <VehicleModelList models={filteredModels} onEdit={handleEdit} onDelete={handleDelete} loading={loading} />
        )}
      </div>
    </div>
  );
